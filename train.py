...
if __name__ == "__main__":
    ...
    # if IS_SCHEDULER:
    #     scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=len(train_dl))
    ...
    for epoch in tqdm.tqdm(range(epoch), "Epoch", position=1):
        ...
        for batch in tqdm.tqdm(train_dl, "Training Batch", position=0, leave=False):
            ...
            fv_batch, bev_batch, speed_batch, pedal_batch, steer_batch = batch
            # print(fv_batch.shape, speed_batch.shape, pedal_batch.shape, steer_batch.shape)
            # print(steer_batch)

            # if IS_AUGMENTATION:
            #     if np.random.random() > AUG_PROBABILITY:
            #         fv_batch = torch.flip(fv_batch, dims=[3])
            #         steer_batch = -steer_batch

            cbs = int(fv_batch.shape[0])

            fv_batch, bev_batch, speed_batch, pedal_batch, steer_batch = fv_batch.to(device), bev_batch.to(device), speed_batch.to(device), pedal_batch.to(device), steer_batch.to(device)
            
            optimizer.zero_grad()
            pedal_pred, steer_pred = model(fv_batch, bev_batch, speed_batch)
